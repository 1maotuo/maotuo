...
Maotuo 就是包在模型外面的控制循环：负责组 prompt、解析模型输出、
...
from .evidence import EvidenceStore, ReleaseGate
from .evolution import EvolutionSignalCollector, EvolutionStore
from .role_skill import RoleSkillManager
...
# Windows 创建 shell 进程时需要这些系统变量；它们不是业务密钥，仍然受白名单控制。
DEFAULT_SHELL_ENV_ALLOWLIST = (
    "HOME", "LANG", "LC_ALL", "LC_CTYPE", "LOGNAME", "PATH", "PWD", "SHELL",
    "TERM", "TMPDIR", "TMP", "TEMP", "USER", "ComSpec", "COMSPEC", "SystemRoot",
...
DEFAULT_FEATURE_FLAGS = {
    ...
    "prompt_cache": True,
    # 以下开关默认保持 Maotuo 现有完整能力；评测才会按组关闭，生产运行不降级安全。
    "coding_prompt": True,
...
class Maotuo:
    def __init__(
        ...
        # Role 只能进一步收缩全局工具权限，不能借此扩大 Maotuo 原有权限。
        self.role_skill = RoleSkillManager(role=role, skills=skills)
        requested_tools = self._normalize_allowed_tools(allowed_tools)
        role_tools = self.role_skill.allowed_tools()
        if requested_tools and role_tools:
            role_set = set(role_tools)
            self.allowed_tools = tuple(name for name in requested_tools if name in role_set)
        else:
            self.allowed_tools = requested_tools or role_tools
        self.run_store = run_store or RunStore(Path(workspace.repo_root) / ".maotuo" / "runs")
        self.evidence_store = EvidenceStore(self.root)
        self.release_gate = ReleaseGate(self.evidence_store, max_retries=2)
        self.evolution = EvolutionSignalCollector()
        self.evolution_store = EvolutionStore(Path(workspace.repo_root) / ".maotuo" / "evolution")
        self.pending_stale_paths = []
    ...
    def build_prefix(self):
        return build_prompt_prefix(
            ...
            tools=self.tools,
            # Role/Skill 会由 ContextManager 放进受保护的 prefix 开头，避免被普通文本挤掉。
            role_prompt="",
        )
    ...
    def shell_env(self):
        filtered = securitylib.shell_env(allowlist=self.shell_env_allowlist, root=self.root)
        if os.name == "nt":
            # 只在真正执行命令的 runtime 边界补齐 Windows shell 变量，
            # 保持 security.shell_env 的纯过滤行为和原有公共契约不变。
            system_root = os.environ.get("SystemRoot") or os.environ.get("WINDIR") or r"C:\Windows"
            filtered.setdefault("SystemRoot", system_root)
            filtered.setdefault("ComSpec", os.environ.get("ComSpec") or os.environ.get("COMSPEC") or str(Path(system_root) / "System32" / "cmd.exe"))
        return filtered
    ...
    def emit_trace(self, task_state, event, payload=None):
        ...
        if event in {
            ...
            "evolution_signal_created",
            "runtime_identity_mismatch",
        }:
            # 安全审计只保留安全相关事件，避免把完整模型上下文复制一份。
            self.run_store.append_audit(task_state, payload)
        return payload
    ...
    def execute_tool(self, name, args):
        ...
        # Evidence 开关仅服务于消融评测；默认始终记录，保证生产运行的发布门禁不被削弱。
        evidence_enabled = self.feature_enabled("evidence_chain")
        if evidence_enabled:
            self.evidence_store.observe_tool(name, args, result.metadata, now())
        if self.current_task_state is not None and evidence_enabled:
            self.emit_trace(
                self.current_task_state,
                "tool_evidence_recorded",
                {"tool": name, "evidence": self.evidence_store.to_dict()},
            )
        return result

    def check_release_gate(self):
        """检查最终答案是否仍然基于最新文件，并准备下一轮提示。"""
        # 只有 Benchmark 的对照组能关闭门禁；默认配置始终走完整 stale 检查。
        if not self.feature_enabled("release_gate"):
            self.pending_stale_paths = []
            return {"allowed": True, "stale_paths": [], "retry_count": 0, "bypassed": True}
        result = self.release_gate.check()
        if result["allowed"]:
            self.pending_stale_paths = []
            return result
        self.pending_stale_paths = list(result["stale_paths"])
        return result

    def _filtered_prompt_strategy(self):
        """读取策略并按当前 Role 过滤潜在冲突的工具指导。"""
        strategy = self.evolution_store.active_strategy()
        if not strategy:
            return None
        proposal = self.role_skill.filter_guidance(strategy.get("proposal", ""))
        if not proposal:
            return None
        filtered = dict(strategy)
        filtered["proposal"] = proposal
        return filtered

    def record_evolution_signal(self, kind, payload=None):
        """记录受控自进化信号，并生成待审批提示候选。"""
        signal = self.evolution.record(kind, payload)
        candidate = self.evolution.candidate(signal)
        return self.evolution_store.create_candidate(candidate, signal)

    def active_prompt_strategy(self):
        """读取人工审批后的提示策略，不触碰工具和安全配置。"""
        return self._filtered_prompt_strategy()
    ...
    def build_report(self, task_state):
        ...
        return {
            ...
            "evolution_signals": list(self.evolution.signals),
            "active_prompt_strategy": self.active_prompt_strategy(),
        }
    ...
    def parse(raw):
        ...
        if "<tool>" in raw and ("<final>" not in raw or raw.find("<tool>") < raw.find("<final>")):
            body = Maotuo.extract(raw, "tool")
            try:
                payload = json.loads(body)
            except Exception:
                return "retry", Maotuo.retry_notice("model returned malformed tool JSON")
            if not isinstance(payload, dict):
                return "retry", Maotuo.retry_notice("tool payload must be a JSON object")
            if not str(payload.get("name", "")).strip():
                return "retry", Maotuo.retry_notice("tool payload is missing a tool name")
            args = payload.get("args", {})
            if args is None:
                payload["args"] = {}
            elif not isinstance(args, dict):
                return "retry", Maotuo.retry_notice()
            return "tool", payload
        if "<tool" in raw and ("<final>" not in raw or raw.find("<tool") < raw.find("<final>")):
            payload = Maotuo.parse_xml_tool(raw)
            if payload is not None:
                return "tool", payload
            return "retry", Maotuo.retry_notice()
        if "<final>" in raw:
            final = Maotuo.extract(raw, "final").strip()
            if final:
                return "final", final
            return "retry", Maotuo.retry_notice("model returned an empty <final> answer")
        raw = raw.strip()
        if raw:
            return "final", raw
        return "retry", Maotuo.retry_notice("model returned an empty response")
    ...
    def parse_xml_tool(raw):
        ...
        if not match:
            return None
        attrs = Maotuo.parse_attrs(match.group("attrs"))
        ...
        for key in ("content", "old_text", "new_text", "command", "task", "pattern", "path"):
            if f"<{key}>" in body:
                args[key] = Maotuo.extract_raw(body, key)
