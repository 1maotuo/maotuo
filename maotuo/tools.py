...
def tool_run_shell(context, args):
    ...
    if timeout < 1 or timeout > 120:
        raise ValueError("timeout must be in [1, 120]")
    filtered_env = context.shell_env()
    # Python 在 Windows 上选择 shell 时读取父进程环境，而不是传入的 env；
    # 显式传入 ComSpec，才能在安全过滤环境后仍然稳定启动命令解释器。
    shell_executable = filtered_env.get("ComSpec") if os.name == "nt" else None
    python_match = re.match(
        r"^\s*(?:\"(?P<double>[^\"]+)\"|'(?P<single>[^']+)'|(?P<plain>\S+))\s+-c\s+(?P<script>.+)$",
        command,
    )
    executable = ""
    if python_match:
        executable = (
            python_match.group("double")
            or python_match.group("single")
            or python_match.group("plain")
        )
    if os.name == "nt" and python_match and executable.lower().endswith(("python.exe", "python")):
        # Windows cmd 不理解 shlex.quote 生成的单引号；对明确的 Python -c
        # 命令走无 shell 直执行路径，避免改变其它 shell 命令的既有行为。
        script = python_match.group("script").strip()
