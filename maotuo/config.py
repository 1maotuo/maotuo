...
def find_runtime_config(start):
    """从当前目录向上查找可选的 maotuo.config.json。"""
    current = Path(start).resolve()
    if current.is_file():
        current = current.parent
    for path in (current, *current.parents):
        config_path = path / "maotuo.config.json"
        if config_path.exists():
            return config_path
    return None


def load_runtime_config(start):
    """加载项目级配置；配置错误时给出明确错误，不静默降级。"""
    config_path = find_runtime_config(start)
    if config_path is None:
        return {}
    payload = json.loads(config_path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("maotuo.config.json must contain a JSON object")
    return payload
