from maotuo import FakeModelClient, Maotuo, SessionStore, WorkspaceContext
...
def test_context_manager_assembles_sections_in_expected_order(tmp_path):
    ...
    assert prompt.index("You are maotuo") < prompt.index("Memory:")
...
def test_high_value_context_pins_latest_failure_change_and_evidence(tmp_path):
    """高价值上下文应优先保留最近失败、修改目标和文件证据。"""
    agent = build_agent(tmp_path, [])
    agent.evidence_store.start_run("context-test")
    agent.evidence_store.observe(tmp_path / "README.md", "read_file", "now")
    agent.record(
        {
            "role": "tool",
            "name": "patch_file",
            "args": {"path": "README.md", "old_text": "demo", "new_text": "fixed"},
            "content": "patched README.md",
    ...
    prompt, metadata = ContextManager(agent).build("修复 README 测试")

    assert prompt.index("Protected Role/Skill policy:") < prompt.index("High-value run context:")
    assert "Latest verification failure:" in prompt
    assert "python -m pytest -q" in prompt
    assert "Latest requested file change:" in prompt
    assert "README.md ->" in prompt
    assert metadata["high_value_context"]["pinned_verification_failure"] is True
    assert metadata["high_value_context"]["latest_change_paths"] == ["README.md"]


def test_high_value_context_pins_rejected_tool_feedback(tmp_path):
    """工具拒绝信息应置顶，提醒模型换一种动作而不是重复调用。"""
    agent = build_agent(tmp_path, [])
    agent.record(
        {
            "role": "tool",
            "name": "read_file",
            "args": {"path": "taskboard/security.py", "start": 1, "end": 200},
            "content": "error: repeated identical tool call for read_file; choose a different tool or return a final answer",
            "created_at": "2026-04-07T10:01:00+00:00",
        }
    )

    prompt, metadata = ContextManager(agent).build("修复命令白名单")

    assert "Latest tool feedback requiring a different action:" in prompt
    assert "repeated identical tool call" in prompt
    assert metadata["high_value_context"]["pinned_tool_failure"] is True
...
def test_context_manager_relevant_memory_can_mix_durable_notes(tmp_path):
    memory_root = tmp_path / ".maotuo" / "memory"
