...
def build_agent(tmp_path, outputs, **kwargs):
    ...
    store = SessionStore(tmp_path / ".maotuo" / "sessions")
...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".maotuo").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".maotuo" not in result
...
def test_welcome_screen_keeps_box_shape_for_long_paths(tmp_path):
    deep = tmp_path / "very" / "long" / "path" / "for" / "the" / "maotuo" / "agent" / "welcome" / "screen"
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-agent"] == "maotuo/0.1"
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            "maotuo.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("maotuo.cli.OpenAICompatibleModelClient") as mock_openai:
            fake_client = mock_openai.return_value
            agent = maotuo_pkg.build_agent(args)
...
def test_build_arg_parser_leaves_provider_unset_for_runtime_resolution(tmp_path):
    args = maotuo_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path)])

    assert args.provider is None


def test_build_arg_parser_accepts_anthropic_provider(tmp_path):
    args = maotuo_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "anthropic"])

    assert args.provider == "anthropic"


def test_build_arg_parser_accepts_deepseek_provider(tmp_path):
    args = maotuo_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
...
def test_build_agent_uses_project_env_provider_when_cli_omitted(tmp_path):
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch(
            ...
            "maotuo.cli.AnthropicCompatibleModelClient",
...
def test_build_agent_prefers_cli_provider_over_project_env_provider(tmp_path):
    ...
    args = maotuo_pkg.build_arg_parser().parse_args(
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch(
            ...
            "maotuo.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("maotuo.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("maotuo.cli.AnthropicCompatibleModelClient") as mock_anthropic:
            maotuo_pkg.build_agent(args)
...
def test_successful_run_persists_run_artifacts_and_stop_reason(tmp_path):
    ...
    runs_root = tmp_path / ".maotuo" / "runs"
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .maotuo/.\n"
    ...
    index_path = tmp_path / ".maotuo" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".maotuo" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".maotuo" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .maotuo/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".maotuo" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".maotuo" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert Maotuo is not None
    assert OllamaModelClient is not None
    assert SessionStore is not None
    assert WorkspaceContext is not None
    assert Path(maotuo_pkg.__file__).as_posix().endswith("/maotuo/__init__.py")
...
def test_package_import_surface_includes_cli_entrypoints():
    assert callable(maotuo_pkg.main)
    assert callable(maotuo_pkg.build_agent)
    assert callable(maotuo_pkg.build_arg_parser)


def test_module_execution_help_works():
    result = subprocess.run(
        [sys.executable, "-m", "maotuo", "--help"],
        capture_output=True,
        text=True,
    )
