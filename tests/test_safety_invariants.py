...
from maotuo import cli as maotuo_cli
from maotuo.task_state import TaskState
...
def test_cli_build_agent_wires_secret_env_names_from_parser(tmp_path):
    ...
    with patch.dict(os.environ, {"GITHUB_PAT": "ghp-1", "GH_PAT": "ghp-2"}, clear=True), patch(
        ...
        args = maotuo_cli.build_arg_parser().parse_args(
            [
                "--cwd",
                str(tmp_path),
                "--approval",
        ...
        agent = maotuo_cli.build_agent(args)
...
def test_cli_build_agent_uses_default_configured_secret_names(tmp_path):
    ...
    with patch.dict(os.environ, {"GH_PAT": "ghp-default-1"}, clear=True), patch(
        ...
        args = maotuo_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = maotuo_cli.build_agent(args)
...
def test_cli_build_agent_loads_project_env_secrets_before_redaction_setup(tmp_path):
    ...
    with patch.dict(os.environ, {}, clear=True), patch("maotuo.cli.AnthropicCompatibleModelClient", DummyModelClient):
        args = maotuo_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
        agent = maotuo_cli.build_agent(args)
...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        ...
        clear=True,
    ), patch("maotuo.cli.OllamaModelClient", DummyModelClient):
        args = maotuo_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = maotuo_cli.build_agent(args)
...
def test_bound_tool_methods_delegate_into_tools_module(tmp_path):
    ...
    with patch("maotuo.tools.subprocess.run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "maotuo.runtime"

    with patch("maotuo.tools.tool_delegate", return_value="toolkit-delegate") as fake_delegate:
